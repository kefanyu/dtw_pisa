...
```{r}
dtw_reading <- as.matrix(dtw_reading)

idx <- which(is.na(dtw_reading), arr.ind=TRUE)
dtw_reading[idx] <- rowMeans(dtw_reading, na.rm=TRUE)[idx[,1]]

dtw_reading <- as.data.frame(dtw_reading)
```
...
dtw_math <- as.matrix(dtw_math)

idx2 <- which(is.na(dtw_math), arr.ind=TRUE)
dtw_math[idx2] <- rowMeans(dtw_math, na.rm=TRUE)[idx2[,1]]

dtw_math <- as.data.frame(dtw_math)
...
dtw_sci <- as.matrix(dtw_sci)

idx3 <- which(is.na(dtw_sci), arr.ind=TRUE)
dtw_sci[idx3] <- rowMeans(dtw_sci, na.rm=TRUE)[idx3[,1]]

dtw_sci <- as.data.frame(dtw_sci)
...
write_xlsx(list("Reading" = reading_dist_df, "Math" = math_dist_df, "Science" = sci_dist_df),"../data_all/dtw_gap.xlsx")
```