...
dtw and euclidean matrix genenration
...
```{r}
reading_clust_opt_eu <- tsclust(reading_lst, type = "partitional", k = 4, distance = "Euclidean", centroid = "pam",seed = 725)
```
...
write_xlsx(list("DTW" = reading_dtw_df, "Euclidean" = reading_eu_df),"../../data/dtw/reading_oecd_distance.xlsx")
```
...
```{r}
math_clust_opt_eu <- tsclust(math_lst, type = "partitional", k = 4, distance = "Euclidean", centroid = "pam",seed = 725)
```
...
```{r}
write_xlsx(list("DTW" = math_dtw_df, "Euclidean" = math_eu_df),"../../data/dtw/math_oecd_distance.xlsx")
```
...
```{r}
sci_clust_opt_eu <- tsclust(sci_lst, type = "partitional", k = 4, distance = "Euclidean", centroid = "pam",seed = 725)
```
...
```{r}
write_xlsx(list("DTW" = sci_dtw_df, "Euclidean" = sci_eu_df),"../../data/dtw/sci_oecd_distance.xlsx")
```
